...
namespace {
    ...
    constexpr const char *kNativeLogTag = "AVEngine";
    ...
    std::string PacketFlagsToString(int flags) {
        ...
        if (flags & AV_PKT_FLAG_CORRUPT) {
            names.emplace_back("corrupt");
        }
        if (flags & AV_PKT_FLAG_DISCARD) {
            names.emplace_back("discard");
        }
        if (flags & AV_PKT_FLAG_TRUSTED) {
            names.emplace_back("trusted");
        }
        if (flags & AV_PKT_FLAG_DISPOSABLE) {
            names.emplace_back("disposable");
        }
    ...
    int ReceiveSoftVideoPackets(SoftVideoEncoder *encoder) {
        ...
        while (true) {
            ...
            const int packetIndex = encoder->packetCount;
            const int64_t deltaPts =
                encoder->lastPacketPts == AV_NOPTS_VALUE || packet->pts == AV_NOPTS_VALUE
                    ? AV_NOPTS_VALUE
                    : packet->pts - encoder->lastPacketPts;
            __android_log_print(
                ANDROID_LOG_DEBUG,
                kNativeLogTag,
                "soft video packet raw index=%d codec=h264 pts=%lld dts=%lld duration=%lld deltaPts=%lld "
                "size=%d flags=%d(%s) keyFrame=%s codecTimeBase=%d/%d ptsSec=%.6f durationSec=%.6f",
                packetIndex,
                static_cast<long long>(packet->pts),
                static_cast<long long>(packet->dts),
                static_cast<long long>(packet->duration),
                static_cast<long long>(deltaPts),
                packet->size,
                packet->flags,
                PacketFlagsToString(packet->flags).c_str(),
                packet->flags & AV_PKT_FLAG_KEY ? "true" : "false",
                encoder->codecContext->time_base.num,
                encoder->codecContext->time_base.den,
                PtsToSeconds(packet->pts, encoder->codecContext->time_base),
                PtsToSeconds(packet->duration, encoder->codecContext->time_base)
            );

            if (packet->pts != AV_NOPTS_VALUE) {
                encoder->lastPacketPts = packet->pts;
            }
            av_packet_unref(packet);
    ...
    std::string OpenSoftVideoEncoder(
        ...
        encoder->codecContext->max_b_frames = encoder->maxBFrames;
        ...
        av_opt_set(
            ...
            "x264-params",
            "repeat-headers=1:annexb=1:bframes=2:b-adapt=1:rc-lookahead=20",
            0
        );
        av_opt_set(
            encoder->codecContext->priv_data,
        ...
        __android_log_print(
            ANDROID_LOG_DEBUG,
            kNativeLogTag,
            "open soft video encoder codec=libx264 output=%s size=%dx%d frameRate=%d "
            "bitrate=%d gop=%d iFrameInterval=%d maxBFrames=%d profile=%s "
            "pixFmt=%d timeBase=%d/%d x264Params=%s",
            encoder->outputPath.c_str(),
            encoder->width,
            encoder->height,
            encoder->frameRate,
            encoder->bitrate,
            encoder->codecContext->gop_size,
            encoder->iFrameInterval,
            encoder->codecContext->max_b_frames,
            NormalizeX264Profile(profile).c_str(),
        ...
        encoder->lastPacketPts = AV_NOPTS_VALUE;
        return "OK: FFmpeg libx264 soft video encoder started with B-frames=2.";
    }
    ...
    int WriteSoftVideoFrame(
        ...
        const int64_t previousFramePts = encoder->lastFramePts;
        int64_t nextPts = ptsUs > 0
        ...
        __android_log_print(
            ANDROID_LOG_DEBUG,
            kNativeLogTag,
            "soft video frame submit index=%d inputPtsUs=%lld codecPts=%lld deltaPts=%lld "
            "timeBase=%d/%d size=%dx%d",
            encoder->frameCount,
            static_cast<long long>(ptsUs),
            static_cast<long long>(frame->pts),
            static_cast<long long>(
                previousFramePts == AV_NOPTS_VALUE ? AV_NOPTS_VALUE : frame->pts - previousFramePts
            ),
            encoder->codecContext->time_base.num,
            encoder->codecContext->time_base.den,
            encoder->width,
    ...
    int ReceiveSoftAacPackets(SoftAacEncoder *encoder) {
        ...
        while (true) {
            ...
            __android_log_print(
                ANDROID_LOG_DEBUG,
                kNativeLogTag,
                "soft AAC packet raw index=%d pts=%lld dts=%lld duration=%lld size=%d flags=%d(%s) "
                "codecTimeBase=%d/%d ptsSec=%.6f durationSec=%.6f",
                encoder->packetCount + emittedPackets,
                static_cast<long long>(packet->pts),
                static_cast<long long>(packet->dts),
                static_cast<long long>(packet->duration),
                packet->size,
            ...
            __android_log_print(
                ANDROID_LOG_DEBUG,
                kNativeLogTag,
                "soft AAC packet mux index=%d pts=%lld dts=%lld duration=%lld size=%d streamIndex=%d "
                "streamTimeBase=%d/%d ptsSec=%.6f durationSec=%.6f",
                encoder->packetCount + emittedPackets,
                static_cast<long long>(packet->pts),
                static_cast<long long>(packet->dts),
                static_cast<long long>(packet->duration),
                packet->size,
                packet->stream_index,
                encoder->audioStream->time_base.num,
                encoder->audioStream->time_base.den,
                PtsToSeconds(packet->pts, encoder->audioStream->time_base),
                PtsToSeconds(packet->duration, encoder->audioStream->time_base)
            );
